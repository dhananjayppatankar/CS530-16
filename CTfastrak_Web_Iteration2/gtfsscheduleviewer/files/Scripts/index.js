...
function load(config) {
  ...
  */
  ...
  _map.fitBounds(config.bounds);

  initIcons();

 

  google.maps.event.addListener(_map, "moveend", callbackMoveEnd);

  google.maps.event.addListener(_map, "zoomend", callbackZoomEnd);
...
function getdirectionSubmit() {

         var souraddre = document.getElementById("saddr").value;

         var destaddre = document.getElementById("daddr").value;

 

         // geocoding to get (lat,lng) of souraddr and destaddr

         var geocoder = new google.maps.Geocoder();

         var sourpos, destpos;

         geocoder.geocode({'address': souraddre}, function(results, status) {
          ...
          if (status === 'OK') {
            ...
            sourpos = results[0].geometry.location;
         ...
         geocoder.geocode({'address': destaddre}, function(results, status) {
          ...
          if (status === 'OK') {
            ...
            destpos = results[0].geometry.location;
            ...
            calculateAndDisplayRoute(directionsService, directionsDisplay, sourpos, destpos);
...
function calculateAndDisplayRoute(directionsService, directionsDisplay, sourpos, destpos) {

    directionsService.route({

      origin: {lat: sourpos.lat(), lng: sourpos.lng()},  // Haight.

      destination: {lat: destpos.lat(), lng: destpos.lng()},  // Ocean Beach.
...
function callbackDisplayStopInfoWindow(marker, data, responseCode) {
  ...
  html = html + "<table><tr><th>Next Departure&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<th>Route</tr>";
  ...
  for (var i=0; i < timeTrips.length; ++i) {
    ...
    //html = html + "<tr onClick='map.closeInfoWindow();selectTrip(\"" + tripid + "\")'>" + "<td>" + service_id + "<td align='right'>" + (timepoint ? "" : "~") + formatTime(time) + "<td>" + tripname + "</tr>";

    html = html + "<tr onClick='map.closeInfoWindow();selectTrip(\"" + tripid + "\")'>" + "<td align='left'>" + (timepoint ? "" : "~") + formatTime(time) + "<td>" + tripname + "</tr>";
  ...
  mapView.setContent(html);

  mapView.open(_map, marker);
...
function displayPolyLine(polyData) {
  ...
  _map.fitBounds(boundsOfPolyLine);
