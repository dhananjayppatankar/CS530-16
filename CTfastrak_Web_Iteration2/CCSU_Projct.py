...
class ScheduleRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
  def do_GET(self):
    ...
    m = re.match(r'/Scripts/(.{1,64})$', path)
    if m and m.group(1):
      try:
        tmp = path[1:]
        f, mime_type = self.OpenFile(tmp)
        return self.handle_static_file_GET(f, mime_type)
      except IOError, e:
        print "Error: unable to open %s" % m.group(1)

    m = re.match(r'/css/(.{1,64})$', path)
    if m and m.group(1):
      try:
        tmp = path[1:]
        f, mime_type = self.OpenFile(tmp)
  ...
  def OpenFile(self, filename):
    ...
    #else:
    mode = 'rb'
    #print(os.path.join(self.server.file_dir, filename))
...
def FindPy2ExeBase():
  ...
  windows_ending = gtfsscheduleviewer.__file__.find('\\library.zip\\')  # @UndefinedVariable
  if windows_ending != -1:
...
'''
  ...
  # server.socket = ssl.wrap_socket (server.socket, keyfile="C:/Users/david/OpenSSL/bin/server.key", certfile="C:/Users/david/OpenSSL/bin/server.crt", server_side=True)
  
  print ("To view, point your browser at ", server.server_address)
